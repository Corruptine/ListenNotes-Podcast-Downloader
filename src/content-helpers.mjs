export const ALL_EPISODES_TIP_TEXT = 'You have seen all episodes of this podcast';
export const ALL_EPISODES_TIP_SELECTOR = '.text-center.my-4.text-muted-color.text-sm';
export const EPISODE_LINK_SELECTOR = [
  'h3 a[href]',
].join(', ');
export const AUDIO_PLAYER_SELECTOR = 'div[data-type="episode-audio-player"]';
export const LOAD_MORE_BUTTON_SELECTOR = [
...
export const EPISODES_PAGINATION_SELECTOR = '#episodes-pagination';

export const SCAN_DECISIONS = {
...
export function normalizeEpisodeTitle(text) {
...
export function extractRssFeedUrl(documentRef, baseUrl) {
  const candidates = [];
...
export function parsePodcastRss(xmlText, baseUrl = '') {
  const xml = String(xmlText || '');
  const channelStart = xml.match(/<channel(?:\s[^>]*)?>/i);
  const channelPrefix = channelStart
    ? xml.slice(channelStart.index + channelStart[0].length, xml.search(/<item(?:\s[^>]*)?>/i) >= 0 ? xml.search(/<item(?:\s[^>]*)?>/i) : xml.length)
    : xml;
...
export function isListenNotesEpisodeUrl(url) {
  try {
    const parsed = new URL(url);
...
export function getUniqueEpisodeAnchors(documentRef, baseUrl) {
...
export function findEpisodeAnchorByUrl(documentRef, baseUrl, episodeUrl) {
...
export function getEpisodeSlug(url) {
  try {
    const parsed = new URL(url);
    const parts = parsed.pathname.split('/').filter(Boolean);
...
export function readEpisodePlayerData(player, baseUrl) {
...
export function getEpisodePlayers(documentRef) {
  if (!documentRef?.querySelectorAll) return [];
  return Array.from(documentRef.querySelectorAll(AUDIO_PLAYER_SELECTOR));
}

export function getEpisodePlayerDataList(documentRef, baseUrl) {
  return getEpisodePlayers(documentRef).map((player) => readEpisodePlayerData(player, baseUrl));
}

export function createEpisodeFromAnchor(documentRef, {
  anchor,
  url,
  baseUrl,
...
export function createEpisodeFromEndpointEpisode(endpointEpisode, baseUrl = '') {
...
export function getEpisodeDedupKey(episode) {
...
export function getEpisodePaginationState(documentRef) {
...
export function findEpisodePlayerForAnchor(documentRef, anchor, episodeUrl) {
...
export function hasAllEpisodesTip(documentRef) {
...
export function findLoadMoreButton(documentRef) {
...
export function getScanDecision({
  hasEndTip,
